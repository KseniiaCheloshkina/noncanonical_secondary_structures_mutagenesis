...
if (add_higher_lev_feats == 1){
  data <- get_higher_level_features(data=data, features_cols = features_cols,
                                    win_len_upper=1000000, path_to_upper_data = data_path_upper)
...
for (target_column in dens_cols){
  ...
  write.csv(df_r2_all, file = paste0(output_path, paste0("regression_full_", add_higher_lev_feats, ".csv")))
  pb$tick()
}
